...
export default [
  ...
  {
    title: ' ¿Cuál es la salida del siguiente código? (Nivel Principiante)',
...
person.greet();`,
  ...
  {
    title: ' ¿Cuál es la salida del siguiente código? (Nivel Intermedio)',
    answer: 'La salida de este código será: "Hello, World!" y "Goodbye, undefined!"',
    explanation: 'En este ejemplo, `object` tiene dos métodos: `greet` y `farewell`. El método `greet` es una función regular que usa `this`, por lo que `this.who` se refiere a la propiedad `who` del objeto `object`, resultando en "Hello, World!". Por otro lado, `farewell` es una arrow function, y las arrow functions no tienen su propio `this`, sino que heredan el `this` del contexto en el que fueron definidas. En este caso, el `this` de `farewell` es el objeto global, donde `who` no está definido, por lo que `this.who` es `undefined`, resultando en "Goodbye, undefined!".',
    code: hljs.highlight(
        `const object = {
    who: 'World',
...
const counter = new Counter();`,
  ...
  {
    title: ' ¿Cuál es la salida del siguiente código? (Nivel Avanzado)',
    answer: 'La salida de este código será: 3',
    explanation: 'En este ejemplo, tenemos una función `callback` que imprime `this.length`. Dentro de `object.method`, llamamos a `arguments[0]()`, donde `arguments` es un objeto parecido a un array que contiene los argumentos pasados a `method`. El primer argumento es `callback`, por lo que `arguments[0]` es `callback`. Cuando `callback` es llamado de esta manera, `this` dentro de `callback` se refiere al objeto `arguments`, que tiene una propiedad `length` que representa el número de argumentos pasados a `method`. En este caso, `arguments` tiene tres elementos: `callback`, `1`, y `2`, por lo que `arguments.length` es `3`.',
    code: hljs.highlight(
        `let length = 4;
function callback() {
